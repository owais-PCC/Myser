...
**Remaining:**
1. Download the fresh artifact, upload to a new Appetize app entry, confirm the app now boots to the actual login screen instead of a blank/stuck loading state, and that the keychain error is gone from the debug logs.
2. Once past that: actually click through Google sign-in and Apple sign-in (Apple's entitlement still isn't wired — see the earlier note in this ticket — so Apple sign-in specifically is still expected to fail/hang, but should now at least reach a real UI to test against).
