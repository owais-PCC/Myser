...
**First CI run failed — fixed:** `npm run build` broke during static-export prerendering with `Firebase: Error (auth/invalid-api-key)`. Root cause: `firebase.ts` reads 6 `NEXT_PUBLIC_FIREBASE_*` env vars at module-init time (which runs during prerendering), and those only exist in the local, gitignored `.env.local` — nothing supplied them to the CI runner. Fixed by setting the 6 values as GitHub Actions repo secrets (`gh secret set`) and adding them as `env:` on the build step. Not a confidentiality concern — they're `NEXT_PUBLIC_*` values that get inlined into the client bundle regardless.

**Remaining:**
1. Re-trigger the workflow from the Actions tab and confirm it produces a valid artifact.
